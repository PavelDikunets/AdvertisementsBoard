...
public class AccountController : ControllerBase
{
    ...
    [HttpPost]
    [AllowAnonymous]
    public async Task<IActionResult> SignUpAsync([FromBody] AccountSignUpDto dto, CancellationToken cancellationToken)
    {
        ...
        _logger.LogInformation("Аккаунт пользователя '{nickName}' успешно зарегистрирован.",
    ...
    {
        _logger.LogInformation("Запрос  аутентификации.");
    ...
    public async Task<IActionResult> GetCurrentByUserIdAsync(CancellationToken cancellationToken)
    {
        ...
        var accounts = await _accountService.GetCurrentByUserId(userId, cancellationToken);
    ...
    [ProducesResponseType(typeof(AccountBlockDto), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ErrorDto), StatusCodes.Status404NotFound)]
    [ProducesResponseType(typeof(ErrorDto), StatusCodes.Status400BadRequest)]
    [HttpPut("{id:guid}/block")]
    [Authorize(Roles = "Administrator")]
    public async Task<IActionResult> BlockByIdAsync(Guid id, [FromBody] AccountBlockDto dto,
        CancellationToken cancellationToken)
    {
        _logger.LogInformation("Запрос установки блоровки аккаунта по Id: '{Id}' в статус '{IsBlocked}'.",
            id, dto.IsBlocked);

        var account = await _accountService.BlockByIdAsync(id, dto, cancellationToken);
